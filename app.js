...
const DEFAULT_CLEANER_SCRIPT = `function main(workbook: ExcelScript.Workbook) {
    ...
    // Delete ranges
    base_File.getRange("J:K").delete(ExcelScript.DeleteShiftDirection.left);
    base_File.getRange("N:O").delete(ExcelScript.DeleteShiftDirection.left);
    base_File.getRange("AB:AG").delete(ExcelScript.DeleteShiftDirection.left);
    base_File.getRange("AD:BD").delete(ExcelScript.DeleteShiftDirection.left);
    base_File.getRange("AG:AP").delete(ExcelScript.DeleteShiftDirection.left);
    base_File.getRange("AI:AM").delete(ExcelScript.DeleteShiftDirection.left);

    // Get the used range for AutoFilter
    let filterRange = base_File.getUsedRange();

    // Toggle auto filter on base_File using the full used range
    base_File.getAutoFilter().apply(filterRange);

    // Apply filter on Customer Chain column (index 2 = 1-based column B)
    base_File.getAutoFilter().apply(filterRange, 2, {
        filterOn: ExcelScript.FilterOn.values,
        values: ["MYNTRA", "NYKAA", "PURPLE", "PURPLLE"]
    });

    // Apply filter on PO Status column (index 28 = 1-based column AI)
    base_File.getAutoFilter().apply(filterRange, 28, {
        filterOn: ExcelScript.FilterOn.values,
        values: ["Open PO"]
    });
    ...
    newSheet.setName("Base File Temp");

    // Paste data
    newSheet.getRange("A1").copyFrom(base_File.getRange("A1:AH211079"), ExcelScript.RangeCopyType.all, false, false);

    // Delete old Base File
    base_File.delete();

    // Rename to "Base File"
    newSheet.setName("Base File");
}`;
...
function initExcelCleanerScript() {
  ...
  if (!script || !script.includes("Base File Temp") || script.includes("requiredCols")) {
