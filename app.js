...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (chainTrigger && chainDropdown) {
    chainTrigger.addEventListener('click', (e) => {
      ...
      // Close OLA Weeks dropdown if open
      const weeksDropdown = document.getElementById('weeks-select-dropdown');
      if (weeksDropdown) weeksDropdown.classList.add('hidden');
    });
  ...
  // OLA Weeks Dropdown Toggle
  const weeksTrigger = document.getElementById('weeks-select-trigger');
  const weeksDropdown = document.getElementById('weeks-select-dropdown');
  
  if (weeksTrigger && weeksDropdown) {
    weeksTrigger.addEventListener('click', (e) => {
      e.stopPropagation();
      if (weeksTrigger.hasAttribute('disabled')) return;
      weeksDropdown.classList.toggle('hidden');
      
      // Close Customer Chain dropdown if open
      const chainDropdown = document.getElementById('chain-select-dropdown');
      if (chainDropdown) chainDropdown.classList.add('hidden');
    });
    
    weeksDropdown.addEventListener('click', (e) => {
      e.stopPropagation();
    });
    
    document.addEventListener('click', () => {
      weeksDropdown.classList.add('hidden');
    });
  }
...
const REQUIRED_COLUMNS = {
  ola: [
    ...
    'week', 'month_year', 'depot', 'pack_tag', 'Average OLA'
  ],
  allocation: [
    'Basepack - Depot', 'Customer Chain', 'PO Date', 'PO Number', 
    'Basepack', 'Basepack Desc', 'Order Value Lacs', 'Alloc Values Lacs', 
...
function handleFileSelect(file, expectedKey) {
  ...
  if (name.includes('ola') && !name.includes('allocation')) {
    detectedKey = 'ola';
  ...
  reader.onload = function(e) {
    try {
      ...
      for (const name of workbook.SheetNames) {
        ...
        // Fallback: If !ref is missing, reconstruct it robustly from cells or !data
        if (sheet && !sheet['!ref']) {
          if (sheet['!data']) {
      ...
      // Fallback: If no sheet was found with data, force-parse the first sheet
      if (sheetData.length === 0 && workbook.SheetNames && workbook.SheetNames.length > 0) {
      ...
      if (sheetData.length > 0) {
        ...
        // If sheetData is empty
        console.warn(`Worksheet parsed as empty for ${detectedKey}`);
        if (detectedKey === 'allocation') {
          alert(`Warning: The Allocation file could not be parsed or contains no data rows. The app will proceed in "OLA-only" mode (assuming no open POs exist). \n\nTip: For large Allocation trackers, saving them as CSV (.csv) before uploading avoids browser memory limits.`);
        } else {
          alert(`Error: The OLA file could not be parsed or contains no data rows. OLA is required to run analysis.`);
          hideLoader();
          return;
        }
      }
      ...
      if (detectedKey === 'ola') {
        ...
        // Extract unique weeks and populate weeks filter dropdown
        const weeks = [...new Set(cleanedData.map(r => String(r.week || '')))].filter(w => w.trim() !== '');
        populateWeeksDropdown(weeks);
        
        // Populate customer chains dropdown from OLA accounts
      ...
      // Enable process button if OLA data is loaded (Allocation is optional)
      if (state.olaData.length > 0) {
...
function runAnalysis() {
  if (state.olaData.length === 0) return;
...
function executeDecisionTreeAnalysis() {
  ...
  // 1. Pre-process Allocation POs and group by (Basepack - Depot, Customer Chain)
  const allocationByBpChain = new Map();
  state.filteredAllocation.forEach(po => {
    // Robustly reconstruct Basepack - Depot from po
    const bp = String(po.Basepack || '').trim();
    const depot = String(po.Depot || '').trim();
    let bpDepot = po['Basepack - Depot'] ? String(po['Basepack - Depot']).trim() : '';
    if (!bpDepot || bpDepot.startsWith('=')) {
      bpDepot = bp && depot ? `${bp} - ${depot}` : (bp || '');
    }
    const chain = getCanonicalChainName(po['Customer Chain']);
    if (!bpDepot || !chain) return;
    
    const key = `${bpDepot}_${chain}`;
  ...
  // 2. Identify all (Basepack - Depot, chain) groups in OLA globally to match POs
  ...
  state.filteredOla.forEach(row => {
    ...
    const bpDepot = `${bp} - ${depot}`;
    const chain = getCanonicalChainName(row.account);
    if (!bp || !chain || !depot) return;
  ...
  // 3. Group OLA by (Basepack - Depot, chain, location) to compute average OLA (For Excel)
  const bpChainLocGroups = new Map(); // key: "bpDepot_chain_loc" -> { bpDepot, basepack, depot, chain, location, rows: [] }
  state.filteredOla.forEach(row => {
    const bp = String(row.basepack || '').trim();
    const depot = String(row.depot || '').trim();
    const bpDepot = `${bp} - ${depot}`;
    ...
    if (!bp || !chain || !loc || !depot) return;
    
    const key = `${bpDepot}_${chain}_${loc}`;
  ...
  bpChainLocGroups.forEach(grp => {
    const bpDepot = grp.bpDepot;
    const bp = grp.basepack;
    const depot = grp.depot;
    const chain = grp.chain;
    const loc = grp.location;
    const bpChainKey = `${bpDepot}_${chain}`;
    const avgOla = grp.rows.reduce((sum, r) => sum + (parseOlaPercentage(r['Average OLA']) || 0), 0) / grp.rows.length;
    
    if (avgOla < 80) {
      const candidatePOs = allocationByBpChain.get(bpChainKey) || [];
      const hasOpenPo = candidatePOs.some(po => {
  ...
  bpChainGroups.forEach((rows, key) => {
    ...
    const bpDepot = parts[0];
    const chain = parts[1];
    
    const bpDepotParts = bpDepot.split(' - ');
    const bp = bpDepotParts[0] || '';
    const depot = bpDepotParts[1] || '';
    
    const avgOla = rows.reduce((sum, r) => sum + (parseOlaPercentage(r['Average OLA']) || 0), 0) / rows.length;
    if (avgOla < 80) {
      ...
      const bpObj = {
        ...
        brand: firstRow.brand || '',
        small_c: firstRow.small_c || '',
        packTag: String(firstRow.pack_tag || '').trim() || 'UNTAGGED',
        avgOla: avgOla,
        rows: rows,
      ...
      const hasOpenPo = bpObj.pos.some(po => {
        const status = String(po['PO Status'] || '').trim().toLowerCase();
        return status === 'open po' || status === 'open';
      });
      
      const isDemand = !hasOpenPo;
      if (isDemand) {
        ...
        // Calculate average Fill Rate (Method A: Value-Weighted across open POs)
        let totalAllocVal = 0;
        let totalOrderVal = 0;
        openPos.forEach(po => {
          const allocVal = parseNumber(po['Alloc Values Lacs']);
          const orderVal = parseNumber(po['Order Value Lacs']);
          totalAllocVal += allocVal;
          totalOrderVal += orderVal;
        });
        const avgFillRate = totalOrderVal > 0 ? (totalAllocVal / totalOrderVal) : null;
        bpObj.avgFillRate = avgFillRate;
        
        // Threshold check: PO Ageing > 7 days or Fill Rate < 0.8 (80%)
        const hasAgeingIssue = avgAgeing !== null && avgAgeing > 7;
        const hasFillRateIssue = avgFillRate !== null && avgFillRate < 0.8;
        
        bpObj.hasAgeingIssue = hasAgeingIssue;
        bpObj.hasFillRateIssue = hasFillRateIssue;
        bpObj.isProblem = hasAgeingIssue || hasFillRateIssue;
      }
    }
  });
  
  // 6. Render Interactive Tree
...
function selectTreeBranch(branch) {
  ...
  if (branch === 'demand') {
    ...
    if (cardSupply) cardSupply.classList.remove('active');
    renderBranchPackTags('demand', accData.demand);
  } else if (branch === 'supply') {
    if (cardSupply) cardSupply.classList.add('active');
    if (cardDemand) cardDemand.classList.remove('active');
    renderSupplyIssueBuckets(accData.supply.all.filter(bp => bp.isProblem));
  }
}

function renderSupplyIssueBuckets(basepacks) {
  const rowSub = document.getElementById('tree-row-subbranches');
  const connSub = document.getElementById('conn-to-subbranches');
  const gridSub = document.getElementById('grid-subbranches');
  const lblSubTitle = document.getElementById('lbl-subbranch-title');
  
  if (!rowSub || !connSub || !gridSub) return;
  
  lblSubTitle.textContent = "Supply Issue Buckets";
  gridSub.innerHTML = '';
  
  // Hide details table and Row 4 classifications initially
  const rowCls = document.getElementById('tree-row-classifications');
  const connCls = document.getElementById('conn-to-classifications');
  if (rowCls) rowCls.classList.add('hidden');
  if (connCls) connCls.classList.add('hidden');
  ...
  const ageingList = basepacks.filter(bp => bp.hasAgeingIssue);
  const fillRateList = basepacks.filter(bp => bp.hasFillRateIssue);
  
  // Create PO Ageing Card
  const ageingCard = document.createElement('div');
  ageingCard.className = 'tree-node-card';
  ageingCard.style.borderWidth = '1.5px';
  ageingCard.innerHTML = `
    <div class="node-card-icon color-red" style="background: rgba(239, 68, 68, 0.15);">
      <i data-lucide="clock"></i>
    </div>
    <div class="node-card-title">PO Ageing Issues</div>
    <div class="node-card-desc">PO Ageing > 7 Days</div>
    <div class="node-card-badge bg-red">${ageingList.length}</div>
  `;
  
  ageingCard.addEventListener('click', () => {
    // Highlight card
    const cards = gridSub.querySelectorAll('.tree-node-card');
    cards.forEach(c => c.classList.remove('active'));
    ageingCard.classList.add('active');
    
    state.activeSupplyClass = 'supplyAgeing';
    // Hide details table first
    if (rowDetails) rowDetails.classList.add('hidden');
    if (connDetails) connDetails.classList.add('hidden');
    
    renderBranchPackTags('supply', ageingList);
  });
  
  // Create Fill Rate Card
  const fillRateCard = document.createElement('div');
  fillRateCard.className = 'tree-node-card';
  fillRateCard.style.borderWidth = '1.5px';
  fillRateCard.innerHTML = `
    <div class="node-card-icon color-orange" style="background: rgba(249, 115, 22, 0.15);">
      <i data-lucide="percent"></i>
    </div>
    <div class="node-card-title">Fill Rate Issues</div>
    <div class="node-card-desc">Fill Rate < 80%</div>
    <div class="node-card-badge bg-orange">${fillRateList.length}</div>
  `;
  
  fillRateCard.addEventListener('click', () => {
    // Highlight card
    const cards = gridSub.querySelectorAll('.tree-node-card');
    cards.forEach(c => c.classList.remove('active'));
    fillRateCard.classList.add('active');
    
    state.activeSupplyClass = 'fillRate';
    // Hide details table first
    if (rowDetails) rowDetails.classList.add('hidden');
    if (connDetails) connDetails.classList.add('hidden');
    
    renderBranchPackTags('supply', fillRateList);
  });
  
  gridSub.appendChild(ageingCard);
  gridSub.appendChild(fillRateCard);
  
  rowSub.classList.remove('hidden');
  connSub.classList.remove('hidden');
  if (window.lucide) window.lucide.createIcons();
  
  setTimeout(() => {
    rowSub.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  }, 50);
}

function renderBranchPackTags(branchType, basepacks) {
  const isDemand = branchType === 'demand';
  
  const targetGrid = isDemand 
    ? document.getElementById('grid-subbranches') 
    : document.getElementById('grid-classifications');
  const targetRow = isDemand 
    ? document.getElementById('tree-row-subbranches') 
    : document.getElementById('tree-row-classifications');
  const targetConn = isDemand 
    ? document.getElementById('conn-to-subbranches') 
    : document.getElementById('conn-to-classifications');
  const targetLabel = isDemand 
    ? document.getElementById('lbl-subbranch-title') 
    : document.getElementById('lbl-classifications-title');
    
  if (!targetGrid || !targetRow || !targetConn) return;
  
  if (isDemand) {
    targetLabel.textContent = "Demand Pack Tags";
  } else {
    targetLabel.textContent = state.activeSupplyClass === 'fillRate' 
      ? "Supply Fill Rate Pack Tags" 
      : "Supply PO Ageing Pack Tags";
  }
  
  targetGrid.innerHTML = '';
  
  // Hide details table initially when entering tags level
  const rowDetails = document.getElementById('tree-row-details');
  const connDetails = document.getElementById('conn-to-details');
  if (rowDetails) rowDetails.classList.add('hidden');
  if (connDetails) connDetails.classList.add('hidden');
  ...
  if (isDemand) {
    // If demand, make sure Row 4 is hidden
    const rowCls = document.getElementById('tree-row-classifications');
    const connCls = document.getElementById('conn-to-classifications');
    if (rowCls) rowCls.classList.add('hidden');
    if (connCls) connCls.classList.add('hidden');
  ...
  if (basepacks.length === 0) {
    targetGrid.innerHTML = `<div class="text-center text-muted" style="grid-column: 1/-1; padding: 20px 0;">No basepacks found for this tag.</div>`;
    targetRow.classList.remove('hidden');
    targetConn.classList.remove('hidden');
    return;
  }
  
  // Group basepacks by packTag
  const groups = {};
  basepacks.forEach(bp => {
    const tag = bp.packTag || 'UNTAGGED';
    if (!groups[tag]) {
      groups[tag] = [];
    }
    groups[tag].push(bp);
  ...
  tags.forEach(tag => {
    ...
    const card = document.createElement('div');
    card.className = 'tree-node-card';
    card.dataset.subcategory = tag;
    card.style.borderWidth = '1.5px';
    
    // Choose custom tag styles/icons
    let color = '#38bdf8'; // Default Sky Blue
    let bgClass = 'rgba(56, 189, 248, 0.15)';
    let icon = 'tag';
    
    const cleanTag = tag.toUpperCase();
    if (cleanTag.includes('LAUNCH')) {
      color = '#a855f7'; // Purple
      bgClass = 'rgba(168, 85, 247, 0.15)';
      icon = 'rocket';
    } else if (cleanTag.includes('TOP')) {
      color = '#eab308'; // Amber/Gold
      bgClass = 'rgba(234, 179, 8, 0.15)';
      icon = 'crown';
    } else if (cleanTag.includes('BHP')) {
      color = '#ec4899'; // Pink
      bgClass = 'rgba(236, 72, 153, 0.15)';
      icon = 'sparkles';
    } else if (cleanTag.includes('REST')) {
      color = '#64748b'; // Slate
      bgClass = 'rgba(100, 116, 139, 0.15)';
      icon = 'archive';
    }
    
    card.innerHTML = `
      <div class="node-card-icon" style="color: ${color}; background: ${bgClass};">
        <i data-lucide="${icon}"></i>
      </div>
      <div class="node-card-title">${tag}</div>
      <div class="node-card-desc">${isDemand ? 'Demand' : (state.activeSupplyClass === 'fillRate' ? 'Fill Rate' : 'PO Ageing')} Basepacks</div>
      <div class="node-card-badge" style="background: ${color};">${count}</div>
    `;
    
    card.addEventListener('click', () => {
      // Highlight card
      const cards = targetGrid.querySelectorAll('.tree-node-card');
      cards.forEach(c => {
        if (c === card) {
      ...
      state.activeSubcategory = tag;
      renderDetailsTable(isDemand ? 'demand' : state.activeSupplyClass, list);
    });
    
    targetGrid.appendChild(card);
  });
  
  // Show target row and connector
  targetRow.classList.remove('hidden');
  targetConn.classList.remove('hidden');
  if (window.lucide) window.lucide.createIcons();
  ...
  if (basepacks.length === 0) {
    ...
    if (type === 'demand') {
      ...
      basepacks.forEach(bp => {
        ...
        tr.innerHTML = `
          <td class="font-semibold">${bp.bpDepot}</td>
          <td>${bp.desc}</td>
...
async function downloadExcelWorkbook() {
  ...
  setTimeout(async () => {
    try {
      ...
      // Extract unique depots from the demand data
      const depots = [...new Set(state.treeData.demand.map(bp => bp.depot))].sort();

      // Standard and custom pack tags
      const packTagsSet = new Set(state.treeData.demand.map(bp => bp.packTag));
      const standardTags = ['LAUNCH', 'TOP PACK', 'BHP', 'REST PORTFOLIO', 'UNTAGGED'];
      const packTags = standardTags.filter(tag => packTagsSet.has(tag));
      packTagsSet.forEach(tag => {
        if (!packTags.includes(tag)) packTags.push(tag);
      });

      const summaryHeaders = ['pack_tag', 'count', ...depots, 'average OLA'];
      wsSummary.addRow(summaryHeaders);

      packTags.forEach(tag => {
        const tagPacks = state.treeData.demand.filter(bp => bp.packTag === tag);
        const count = tagPacks.length;
        if (count === 0) return;

        const rowData = [tag, count];

        // OLA values for each depot
        depots.forEach(depotName => {
          const depotPacks = tagPacks.filter(bp => bp.depot === depotName);
          if (depotPacks.length > 0) {
            const avg = depotPacks.reduce((sum, bp) => sum + bp.avgOla, 0) / depotPacks.length;
            rowData.push(Number(avg.toFixed(2)));
          } else {
            rowData.push(""); // empty if no combination
          }
        });

        // Overall average OLA for this pack tag
        const overallAvg = tagPacks.reduce((sum, bp) => sum + bp.avgOla, 0) / count;
        rowData.push(Number(overallAvg.toFixed(2)));

        wsSummary.addRow(rowData);
      });

      styleWorksheet(wsSummary, summaryHeaders.length);
      ...
      const headers1 = [
        'Account', 'Basepack - Depot', 'Basepack', 'Depot', 'basepack_desc', 'Customer Chain', 'small_c', 'brand', 'location', 
        'pack_tag', 'average ola'
      ];
      ...
      styleWorksheet(ws1, headers1.length);

      // Helper function to style worksheet
      function styleWorksheet(ws, colCount) {
        // Style Header Row
        ...
        for (let r = 2; r <= rowCount; r++) {
          ...
          for (let c = 1; c <= colCount; c++) {
            ...
            if (ws.name === "Summary") {
              ...
              if (colHeaderClean === 'basepack' || colHeaderClean === 'po number' || colHeaderClean === 'basepack - depot') {
...
async function downloadSupplyExcelWorkbook() {
  const problemPacks = [...state.treeData.supply.all.filter(bp => bp.isProblem)];
  ...
  setTimeout(async () => {
    try {
      ...
      // Helper function to style a supply worksheet
      function styleSupplyWorksheet(wsToStyle, headersList) {
        // Style Header Row
        const headerRow = wsToStyle.getRow(1);
        headerRow.height = 26;
        for (let c = 1; c <= headersList.length; c++) {
          const cell = headerRow.getCell(c);
          cell.fill = headerFill;
          cell.font = headerFont;
          cell.alignment = { vertical: 'middle', horizontal: 'center' };
        ...
        // Style Data Rows
        const rowCount = wsToStyle.rowCount;
        for (let r = 2; r <= rowCount; r++) {
          const row = wsToStyle.getRow(r);
          row.height = 20;
          const isEven = (r % 2 === 0);
          ...
          for (let c = 1; c <= headersList.length; c++) {
            ...
            const colHeader = headersList[c - 1];
            const val = cell.value;
            const colHeaderClean = colHeader.trim().toLowerCase();
            ...
            } else if (colHeaderClean.includes('ola') || colHeaderClean.includes('fill rate') || colHeaderClean.includes('rate')) {
              cell.alignment = { vertical: 'middle', horizontal: 'right' };
              if (typeof val === 'number') {
                if (colHeaderClean.includes('ola')) {
                  cell.numFmt = '#,##0.0';
                } else {
                  cell.numFmt = '0.0%';
                }
        ...
        for (let c = 1; c <= headersList.length; c++) {
          let maxLength = 10;
          wsToStyle.getColumn(c).eachCell({ includeEmpty: true }, (cell) => {
            const cellVal = cell.value;
            if (cellVal !== undefined && cellVal !== null) {
              let strLen = 0;
              if (cellVal instanceof Date) {
                ...
                strLen = (cellVal * 100).toFixed(1).length + 1;
              } else if (typeof cellVal === 'number') {
                strLen = cellVal.toFixed(1).length;
              } else {
                strLen = String(cellVal).length;
          ...
          wsToStyle.getColumn(c).width = Math.min(maxLength + 4, 30);
        }
      }
 
      // -------------------------------------------------------------
      // SHEET 1: Supply Ageing Issues
      // -------------------------------------------------------------
      const ws = workbook.addWorksheet("Supply Ageing Issues");
      ws.views = [{ showGridLines: true }];
      
      const headers = [
        'Account', 'Basepack - Depot', 'basepack', 'depot', 'basepack_desc', 'Customer Chain', 'small_c', 'average ola 2 weeks', 'po date', 'Po ageing', 'po number', 'po ageing bucket'
      ];
      ...
      styleSupplyWorksheet(ws, headers);

      // -------------------------------------------------------------
      // SHEET 2: Supply Fill Rate Issues
      // -------------------------------------------------------------
      const wsFill = workbook.addWorksheet("Supply Fill Rate Issues");
      wsFill.views = [{ showGridLines: true }];

      const headersFill = [
        'Account', 'Basepack - Depot', 'basepack', 'depot', 'basepack_desc', 'Customer Chain', 'small_c', 'average ola 2 weeks', 'po date', 'fill rate'
      ];

      wsFill.addRow(headersFill);

      problemPacks.forEach(bpObj => {
        if (!bpObj.hasFillRateIssue) return;
        const openPos = bpObj.openPos || [];
        ...
        openPos.forEach(po => {
          ...
          const orderVal = parseNumber(po['Order Value Lacs']);
          const allocVal = parseNumber(po['Alloc Values Lacs']);
          const poFillRate = orderVal > 0 ? (allocVal / orderVal) : 0.0;
      ...
      styleSupplyWorksheet(wsFill, headersFill);
      
      // Generate buffer and trigger download
      const buffer = await workbook.xlsx.writeBuffer();
      const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      
      const filename = `SUPPLY_ISSUES.xlsx`;
...
function parseOlaPercentage(val) {
  if (val === undefined || val === null || val === '') return 0;
  if (typeof val === 'number') {
...
function populateCustomerChainsDropdown() {
  ...
  // Extract unique accounts from OLA data if loaded
  if (state.olaData && state.olaData.length > 0) {
    state.olaData.forEach(row => {
      const chain = String(row.account || '').trim();
      if (chain) {
  ...
  // Fallback to Allocation data if OLA is not loaded
  if (chainsSet.size === 0 && state.allocationData && state.allocationData.length > 0) {
  ...
  state.selectedChains = []; // default none selected
...
function populateWeeksDropdown(weeks) {
  // Sort weeks descending (e.g. Week 24, Week 23, etc.)
  const weekRegex = /(?:week\s*)?(\d+)(?:[-\s](\d{4}))?/i;
  const parseWeekKey = (wStr) => {
    const match = wStr.match(weekRegex);
    if (match) {
      const weekNum = parseInt(match[1], 10);
  ...
  const sortedWeeks = [...weeks].sort((a, b) => {
    return parseWeekKey(b).localeCompare(parseWeekKey(a));
  });
  
  const listContainer = document.getElementById('weeks-options-list');
  const selectTrigger = document.getElementById('weeks-select-trigger');
  if (!listContainer || !selectTrigger) return;
  ...
  if (sortedWeeks.length === 0) {
    if (selectTriggerText) selectTriggerText.textContent = 'No Weeks Found';
    selectTrigger.setAttribute('disabled', 'true');
    return;
  }
  
  selectTrigger.removeAttribute('disabled');
  state.availableWeeks = sortedWeeks;
  
  // By default, select all weeks
  state.selectedWeeks = []; // default none selected
  updateWeeksTriggerText();
  
  sortedWeeks.forEach(week => {
    const optionDiv = document.createElement('div');
    optionDiv.className = 'custom-select-option';
    optionDiv.dataset.value = week;
    
    const chk = document.createElement('input');
    chk.type = 'checkbox';
    chk.id = `chk-week-${week.replace(/\s+/g, '-')}`;
    chk.checked = false;
    ...
    chk.addEventListener('change', (e) => {
      e.stopPropagation();
      if (chk.checked) {
        if (!state.selectedWeeks.includes(week)) {
      ...
      const chkAll = document.getElementById('chk-weeks-all');
      if (chkAll) {
        chkAll.checked = (state.selectedWeeks.length === state.availableWeeks.length);
      }
      updateWeeksTriggerText();
    });
  ...
  if (chkAll) {
    ...
    newChkAll.addEventListener('change', (e) => {
      ...
      const checked = newChkAll.checked;
      if (checked) {
        state.selectedWeeks = [...state.availableWeeks];
      } else {
        state.selectedWeeks = [];
      }
      
      state.availableWeeks.forEach(week => {
        const input = document.getElementById(`chk-week-${week.replace(/\s+/g, '-')}`);
        if (input) input.checked = checked;
      });
    ...
    const selectAllDiv = document.querySelector('#weeks-select-dropdown .select-all-option');
    if (selectAllDiv) {
      const newSelectAllDiv = selectAllDiv.cloneNode(true);
      selectAllDiv.parentNode.replaceChild(newSelectAllDiv, selectAllDiv);
      
      const chkAllReplaced = document.getElementById('chk-weeks-all');
      newSelectAllDiv.addEventListener('click', (e) => {
        if (e.target !== chkAllReplaced && e.target !== newSelectAllDiv.querySelector('label')) {
          chkAllReplaced.checked = !chkAllReplaced.checked;
          chkAllReplaced.dispatchEvent(new Event('change'));
...
function updateWeeksTriggerText() {
  const selectTrigger = document.getElementById('weeks-select-trigger');
  if (!selectTrigger) return;
  const selectTriggerText = selectTrigger.querySelector('span');
  if (!selectTriggerText) return;
  
  const count = state.selectedWeeks.length;
  const total = state.availableWeeks.length;
  
  if (count === total) {
    selectTriggerText.textContent = `Weeks: All (${total})`;
  } else if (count === 0) {
    selectTriggerText.textContent = `Weeks: None Selected`;
  } else if (count <= 2) {
    selectTriggerText.textContent = `Weeks: ${state.selectedWeeks.join(', ')}`;
  } else {
    selectTriggerText.textContent = `Weeks: ${count} of ${total} Selected`;
  }
  
  // Disable process button if no weeks are selected
  if (count === 0) {
    if (btnProcess) btnProcess.setAttribute('disabled', 'true');
  } else if (state.olaData.length > 0 && state.selectedChains.length > 0) {
    if (btnProcess) btnProcess.removeAttribute('disabled');
  }
}
...
function updateChainTriggerText() {
  ...
  } else if (state.olaData.length > 0) {
...
const DEFAULT_CLEANER_SCRIPT = `function main(workbook: ExcelScript.Workbook) {
	...
	// Delete worksheet quota_Loss_Working
	quota_Loss_Working.delete();
	let quota_Loss_Pivot = workbook.getWorksheet("Quota Loss Pivot");
	// Delete worksheet quota_Loss_Pivot
	quota_Loss_Pivot.delete();
	let sheet1 = workbook.getWorksheet("Sheet1");
	// Delete worksheet sheet1
	sheet1.delete();
	let basepack_Depot_ATP_View = workbook.getWorksheet("Basepack-Depot-ATP View");
	// Delete worksheet basepack_Depot_ATP_View
	basepack_Depot_ATP_View.delete();
	let pivot_Use = workbook.getWorksheet("Pivot Use");
	// Delete worksheet pivot_Use
	pivot_Use.delete();
	...
	// Delete worksheet new_base_use_advance
	new_base_use_advance.delete();
	let new_Base_Use = workbook.getWorksheet("New Base Use");
	// Delete worksheet new_Base_Use
	new_Base_Use.delete();
	let new_Base_Pivot = workbook.getWorksheet("New Base Pivot");
	// Delete worksheet new_Base_Pivot
	new_Base_Pivot.delete();
	let base_File = workbook.getWorksheet("Base File");
	// Delete range I:K on base_File
	base_File.getRange("I:K").delete(ExcelScript.DeleteShiftDirection.left);
	// Delete range K:N on base_File
	base_File.getRange("K:N").delete(ExcelScript.DeleteShiftDirection.left);
	// Delete range Y:AD on base_File
	base_File.getRange("Y:AD").delete(ExcelScript.DeleteShiftDirection.left);
	// Delete range AA:BA on base_File
	base_File.getRange("AA:BA").delete(ExcelScript.DeleteShiftDirection.left);
	// Delete range AD:AM on base_File
	base_File.getRange("AD:AM").delete(ExcelScript.DeleteShiftDirection.left);
	// Toggle auto filter on base_File
	base_File.getAutoFilter().apply(base_File.getRange("AI1"));
	// Delete range AF:AJ on base_File
	base_File.getRange("AF:AJ").delete(ExcelScript.DeleteShiftDirection.left);
}`;
...
function initExcelCleanerScript() {
  ...
  const currentScriptVersion = '3.12.0';
  const savedVersion = localStorage.getItem('ola_allocation_cleaner_script_version');
  
  // Force update if version mismatch, script doesn't exist, or has old elements
  if (!script || savedVersion !== currentScriptVersion || script.includes("requiredCols")) {
    script = DEFAULT_CLEANER_SCRIPT;
    localStorage.setItem('ola_allocation_cleaner_script', DEFAULT_CLEANER_SCRIPT);
    localStorage.setItem('ola_allocation_cleaner_script_version', currentScriptVersion);
  }
...
// -------------------------------------------------------------
const DEFAULT_ATLAN_QUERY = `SELECT * FROM (
    -- ✅ MYNTRA + NYKAA query (no consideration/pack_tag filters)
    SELECT
        "account",
        "basepack",
        "basepack_desc",
    ...
    -- ✅ PURPLLE query (with consideration and pack_tag filters)
    SELECT
        "account",
        "basepack",
        "basepack_desc",
    ...
    WHERE 
        "account" IN ('PURPLLE')
        AND "consideration" IN ('TRUE')
        AND "pack_tag" IN ('LAUNCH', 'TOP PACK', 'BHP')
    GROUP BY  
        "account",
        "basepack",
        "basepack_desc",
...
function initAtlanQueryScript() {
  ...
  if (!query || !query.includes("UNION ALL") || !query.includes('"depot"')) {
