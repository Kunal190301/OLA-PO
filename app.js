// Version: 3.5.1 (Fixed typo support for Pack classification)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (versionEl) {
    versionEl.textContent = '3.5.1 (Latest)';
  }
});
...
const state = {
  ...
  // Matching lookup (basepack -> Allocation PO)
  matchingMap: new Map(),
...
const REQUIRED_COLUMNS = {
  ola: [
    ...
    'week', 'month_year', 'depot', 'pack_tag', 'MRP', 'Average OLA', 'Pack classification'
  ],
  allocation: [
...
function executeDecisionTreeAnalysis() {
  ...
  // 1. Pre-process Allocation POs and group by Basepack
  const allocationByBasepack = new Map();
  state.filteredAllocation.forEach(po => {
    const bp = String(po.Basepack || '').trim();
    if (!bp) return;
    
    if (!allocationByBasepack.has(bp)) {
      allocationByBasepack.set(bp, []);
    }
    allocationByBasepack.get(bp).push(po);
  });
  
  // 2. Identify all basepacks in OLA globally to match POs
  const basepackGroups = new Map();
  state.filteredOla.forEach(row => {
    const bp = String(row.basepack || '').trim();
    if (!bp) return;
    if (!basepackGroups.has(bp)) {
      basepackGroups.set(bp, []);
    }
    basepackGroups.get(bp).push(row);
  });
  
  const basepacksNoPo = new Set();
  const matchedPosByBasepack = new Map();
  
  basepackGroups.forEach((rows, bp) => {
    const candidatePOs = allocationByBasepack.get(bp) || [];
    if (candidatePOs.length === 0) {
      basepacksNoPo.add(bp);
    } else {
      ...
      matchedPosByBasepack.set(bp, selectedPO);
      state.matchingMap.set(bp, selectedPO); // Store for explorer reference
    }
  });
  
  // 3. Group OLA by (basepack, location) to compute average OLA (For Excel)
  const bpLocGroups = new Map(); // key: "bp_loc" -> { basepack, location, rows: [] }
  state.filteredOla.forEach(row => {
    const bp = String(row.basepack || '').trim();
    ...
    bpLocGroups.get(key).rows.push(row);
  });
  
  // 4. Filter and build detailedBreakdown (For Excel)
  bpLocGroups.forEach(grp => {
    ...
    if (avgOla < 80) {
      ...
      if (basepacksNoPo.has(bp)) {
        shouldRaise = true;
      } else {
        const po = matchedPosByBasepack.get(bp);
      ...
      if (shouldRaise) {
        ...
        state.detailedBreakdown.push({
          'Basepack': bp,
          'basepack_desc': firstRow.basepack_desc || (matchedPosByBasepack.has(bp) ? matchedPosByBasepack.get(bp)['Basepack Desc'] : ''),
          'small_c': firstRow.small_c || '',
          'brand': firstRow.brand || '',
          'location': loc,
          'Pack classification': firstRow['Pack classification'] || '',
  ...
  basepackGroups.forEach((rows, bp) => {
    ...
    if (avgOla < 80) {
      ...
      const bpObj = {
        basepack: bp,
        desc: firstRow.basepack_desc || (matchedPosByBasepack.has(bp) ? matchedPosByBasepack.get(bp)['Basepack Desc'] : ''),
        brand: firstRow.brand || '',
        small_c: firstRow.small_c || '',
        classification: (() => {
          const raw = String(firstRow['Pack classification'] || '').trim();
          if (!raw) return '#N/A';
        ...
        avgOla: avgOla,
        rows: rows,
        pos: allocationByBasepack.get(bp) || []
      };
...
function renderInteractiveTree() {
  ...
  const totalSupplyProblemPacks = 
    state.treeData.supply.classifications['A'].length + 
    state.treeData.supply.classifications['B'].length + 
    state.treeData.supply.classifications['C'].length + 
    state.treeData.supply.classifications['#N/A'].length;
  
  if (rootCountEl) rootCountEl.textContent = state.treeData.root.length.toLocaleString();
  if (demandCountEl) demandCountEl.textContent = state.treeData.demand.length.toLocaleString();
  if (supplyCountEl) supplyCountEl.textContent = totalSupplyProblemPacks.toLocaleString();
  ...
  // Clear any active tree states and hide lower rows
...
function renderDemandClassifications() {
  ...
  state.treeData.demand.forEach(bp => {
...
function selectDemandClassification(cls) {
  ...
  const basepacks = state.treeData.demand.filter(bp => bp.classification === cls);
...
function renderSupplyClassificationsDirect() {
  ...
  ['A', 'B', 'C', '#N/A'].forEach(cls => {
    const count = state.treeData.supply.classifications[cls].length;
    const card = document.createElement('div');
    card.className = 'tree-node-card';
    card.dataset.classification = cls;
...
function selectSupplyClassificationDirect(cls) {
  ...
  const basepacks = state.treeData.supply.classifications[cls];
    ...
    if (type === 'demand') {
      ...
      basepacks.forEach(bp => {
        ...
        tr.innerHTML = `
          ...
          <td>${bp.desc}</td>
          <td>${bp.brand}</td>
          <td><span class="badge">${bp.classification}</span></td>
...
async function downloadExcelWorkbook() {
  ...
  setTimeout(async () => {
    try {
      ...
      const headers1 = [
        'Basepack', 'basepack_desc', 'small_c', 'brand', 'location', 
      ...
      state.detailedBreakdown.forEach(row => {
        ws1.addRow([
          row['Basepack'], row['basepack_desc'], row['small_c'], row['brand'], row['location'],
      ...
      if (state.summaryRaisePo.length > 0) {
      ...
      const filename = `PURPLLE_RAISE_PO.xlsx`;
...
async function downloadSupplyExcelWorkbook() {
  ...
  setTimeout(async () => {
    try {
      ...
      const headers = [
        'basepack', 'basepack_desc', 'small_c', 'average ola 2 weeks', 'po date', 'Po ageing', 'po number', 'po ageing bucket'
      ];
      ...
      const filename = `PURPLLE_SUPPLY_AGEING.xlsx`;
